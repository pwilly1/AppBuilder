...
const GENERATION_INSTRUCTIONS = [
  ...
  'Every targetPageKey and redirectPageKey must match a returned page key or a unique semantic alias derived from an existing page title or path, such as Home or /home becoming home.',
  'Give every page one cohesive visualStyle. Use its palette consistently instead of choosing unrelated colors for individual blocks.',
  'Group related blocks into visual sections. Every section may contain only blocks that share the same page or parent grid, and a block may belong to at most one section.',
  'Assign visual roles that match block behavior: hero=heading, editable text=field, static text=heading or body, button=primaryAction or secondaryAction, repeater=list.',
  'Lay out each page on a 16-column by 29-row grid and avoid overlapping sibling blocks.',
  'Treat exact grid coordinates as layout intent. The deterministic compiler owns final placement, sizing, margins, alignment, and section spacing.',
  'Put blocks and sections in the reading order they should use in the finished page.',
  ...
  'Use balanced outer margins, shared edges for related blocks, equal widths for related fields, and a small consistent spacing scale.',
  'Use intro, form, list, split, and actions section patterns as composition intent, not as permission to omit exact block grid positions.',
  'Reserve enough rows for wrapped text, use tighter spacing inside a section, and use larger spacing between sections.',
  'Intentional asymmetry is allowed when it is balanced and represented by a split section; avoid accidental one-column drift and uneven paired columns.',
  ...
  'When correction data is supplied, preserve every page, block, collection, field, action, binding, parent relationship, visual role, section membership, visual style, and semantic content from the previous plan.',
