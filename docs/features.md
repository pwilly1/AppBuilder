...
The editor accepts a bounded page prompt, calls the project-owned backend proposal route, and shows request progress and hourly quota state. It re-parses every returned plan, compiles it into the normal project schema, previews the isolated result, treats model coordinates as layout hints, resolves bindings and actions, rejects genuinely broken references, repairs unreadable generated color combinations, and applies acceptance as one undoable transaction. After successful compilation, the editor captures only the rendered phone preview and runs one multimodal visual-review pass for hierarchy, spacing, balance, and contrast. The backend permits presentation changes while preserving semantic content and behavior, and the frontend compiles and validates the result again. Failed visual review falls back to the original valid proposal. Generated pages also carry a transient page theme, semantic block roles, and section patterns. The compiler sanitizes harmless visual metadata, globally composes each owner grid with consistent gutters and spacing, and resolves unique aliases for existing pages, collections, fields, and editable inputs before requesting a correction. Theme and composition results are stored as normal schema props and grid placements rather than a browser-only styling layer, preserving the web/Android runtime boundary.
...
The backend has a provider-neutral model client, privacy-limited project context, deterministic fake provider, OpenAI Responses API adapter, and authenticated ownership-checked proposal, correction, and visual-review routes. OpenAI output is constrained with a strict JSON schema and then validated by the shared parser. The service never writes projects or stores preview screenshots. Mongo-backed hourly account quotas, request status records, provider token totals, and a project-owned usage summary endpoint protect the real provider without storing prompts or generated plans. The frontend may coordinate two bounded layout corrections plus one visual review after compilation. Corrections preserve page and block sets, parent relationships, and non-layout content; visual review additionally permits presentation changes while preserving app behavior. Neither path splits pages.
...
- GenAI supports authenticated natural-language page proposals, deterministic theme/layout/color/composition repair, one screenshot-backed presentation review, isolated preview, one-transaction apply, hourly account quotas, and up to two bounded correction attempts. Broad live-provider evaluation, prompt-to-app, AI editing, billing plans, and systematic Android parity coverage are not complete.
