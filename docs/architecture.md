...
Button supports no action, navigation, submission, current-user record update/delete, generated-app authentication, URL, and page-variable actions; Icon and Image support their applicable tap actions. Generated-app accounts are stored separately from builder accounts and scoped by project ID. Web stores one runtime token per project in local storage; Android stores the matching token in app preferences. `setPageState` can assign a fixed text value or the current value of an editable Text block referenced by stable block ID. A Submit Data button remains the app-data source identity. Update and Delete Data resolve the signed-in user's newest owned record, enforce collection policies on the backend, confirm destructive deletion, and refresh current-user bindings after success. Web and Android have separate executors over the same action JSON.
...
Generated-app identity now supports signup, login, logout, stable app-user IDs, project-scoped runtime JWTs, optional ownership metadata on authenticated submissions, current-user display bindings, and owner-scoped list/update/delete behavior governed by collection access rules. Button mutation actions currently target only the signed-in user's newest owned record. Profile/password recovery, arbitrary end-user-selected record mutation, app-state actions, generic page parameters, and filters/sorting are not implemented. Page-state values reset when the page runtime is recreated and are not persisted as hosted app data. The architecture keeps static properties, runtime bindings, and event actions separate; blocks must not become independent database query clients.
