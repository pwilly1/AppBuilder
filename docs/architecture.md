...
The editor implements this boundary with a production `useAiGeneration` hook: request cancellation, quota status, controlled errors, compilation, rendered-preview capture, visual review, and preview operate without mutating the current project; stale proposals cannot be accepted; and only explicit acceptance enters `applyProjectTransaction`. The browser captures only the isolated generated phone preview with placeholder record data. The versioned plan contract, strict parser, and capability catalog are published internally from `@apptura/shared/ai`, while editor-state compilation, layout repair, final project validation, screenshot capture, and preview remain frontend-owned.

The backend exposes authenticated, ownership-checked proposal, correction, and visual-review routes behind a narrow model-client interface. It supports a deterministic fake provider and a backend-only OpenAI adapter with controlled text/image input, strict structured output, request timeout configuration, refusal/incomplete-response handling, and validation through `@apptura/shared/ai`. Mongo-backed account quotas are reserved before every provider call, and prompt-free usage records preserve request status, latency, and provider token totals. The frontend performs another parser pass before compilation, coordinates at most two structural corrections, and requests at most one visual review after a valid render. The visual-review contract allows presentation changes but restores protected pages, collections, block semantics, actions, bindings, access rules, and hierarchy. Preview images are size/type validated, kept in memory, and not persisted.
